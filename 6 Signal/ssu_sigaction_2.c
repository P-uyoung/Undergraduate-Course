...
void ssu_check_pending(int signo, char *signame); 	// 시그널이 블록된 상태인지 확인하는 함수
...
int main(void) {
	...
	if (sigaction(SIGUSR1, &sig_act, NULL) != 0) {		// 시그널에 관한 액션 변경 
...
void ssu_check_pending(int signo, char *signame) {
	...
	else printf("%s signals are not pending\n", signame);
} 

void ssu_signal_handler(int signo) { 
	printf("in ssu_signal_handlerfunction\n"); 
}
