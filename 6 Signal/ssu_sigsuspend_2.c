...
int main(void)
{
	...
	if (sigprocmask(SIG_BLOCK, &new_mask, &old_mask) < 0) {		// SIG_BLOCK: 시그널 블록
	...
	if (sigprocmask(SIG_SETMASK, &old_mask, NULL) < 0) {		// SIG_SETMASK: 시그널 블록 해제
		fprintf(stderr, "SIG_SETMAST() error\n");
		exit(1);
	}
...
void ssu_print_mask(const char *str) {		// 해당 시그널이 존재하는지 확인하는 함수
