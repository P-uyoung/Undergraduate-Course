...
int ssu_do_grep(void) {
	...
	if (S_ISDIR(statbuf.st_mode) == 0) {		//일반파일인 경우 grep command 실행
		sprintf(command, "%s %s", grep_cmd, pathname);
		printf("%s : \n", pathname);
		system(command);		//grep command 를 실행하여 해당 키워드가 파일에 존재하는지 판단
		return 0;
	}
	...
	*ptr = '\0';
	...
	//디렉토리 파일인 경우, 해당 디렉토리를 읽는다
	if ((dp = opendir(pathname)) == NULL) {
		fprintf(stderr, "opendir error fir %s\n", pathname);
		return 0;
	}
